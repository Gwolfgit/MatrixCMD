    ...
    $("#save-changes").click(function () {
        ...
        if($('#switch-dhcp').is(':checked')){
           req.submit(CMD.getHex(CMD.DHCP, true)); 
            ...
            // if(currentMac != smac){
            //   req.submit(CMD.getHex(CMD.Mac, true, mac[0] * 0x100 + mac[1], mac[2] * 0x100 + mac[3], mac[4] * 0x100 + mac[5]));
            // }
...
//          showLoader("Get status...");
    ...
    if (cmdType === 0x08 && cmdIndex === 0x0a) {
        ...
        /**
         * 重置指令有问题：
         * 指令发送后，获取到的是上一条指令执行的结果；
         * 需要再次发送该指令，才会正确收到该条指令执行的结果。
         *
         * 处理方案：
         * 在第一次发送的时候就打开等待面板，
         * 第一次发送指令肯定无效（而实际可能是有效的），所以1s后再次发送确认
         * 第二次发送后，如果返回正确的结果， 就说明指令执行成功，开始查询状态信息
         */
